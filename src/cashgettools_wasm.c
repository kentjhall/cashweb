...
CW_STATUS CWG_WA_get_by_id_bitdb(const char *id, const char *bitdbNode, char *mimeStr, int fd) {
...
/*
 * gets by CashWeb ID via specified REST HTTP endpoint
 * writes MIME type to given memory location and file to descriptor
 */
EMSCRIPTEN_KEEPALIVE
CW_STATUS CWG_WA_get_by_id_rest(const char *id, const char *rest, char *mimeStr, int fd) {
	struct CWG_params params;
	char mimeBuf[CWG_MIMESTR_BUF]; mimeBuf[0] = 0;
	init_CWG_params(&params, NULL, NULL, rest, &mimeBuf);	
	params.datadir = "/data";
	CWG_err_stream = stderr;
